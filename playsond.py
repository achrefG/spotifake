...
def playsond(path_son ,path_cover):
    ...
    while lancer: # tant que lancer est vrai 
        ...
        for event in pygame.event.get(): # pour event variant en fonction de ce qu'il ce passe dans la fenêtre 
            if event.type == pygame.QUIT: # si l'evenement est : la fenetre est quitter 
                ...
                if event.key == pygame.K_q:#fermer la fenetre et revenir en arriere
                   son.unload()
                   lancer=False
                if event.key == pygame.K_ESCAPE:#fermer la fenetre et revenir en arriere
                    son.unload()
                    lancer=False
                ''' 
